...
export function createAmbience() {
  ...
  let engineGain = null;
  let engineOsc = null;
  const state = { density: 0.7, raining: false, hour: 12, engine: 0 };
  ...
  const applyLevels = () => {
    ...
    if (engineGain && engineOsc) {
      const e = muted ? 0 : Math.max(0, Math.min(1, state.engine || 0));
      engineGain.gain.setTargetAtTime(e * 0.22, t, 0.15);
      engineOsc.frequency.setTargetAtTime(55 + e * 90, t, 0.2);
    }
  };
  ...
  const start = async () => {
    ...
    try {
      ...
      engineOsc.connect(engFilt).connect(engineGain).connect(master);
      engineOsc.start();
  ...
  return {
    start,
    set({ density, raining, hour, music, engine }) {
      ...
      if (engine !== undefined) state.engine = engine;
      applyLevels();
    },
    toggleMute() {
      muted = !muted;
