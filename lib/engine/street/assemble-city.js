...
export async function assembleCityFromBuild(built, ctx) {
  ...
  for (const fp of footprints) addFootprint(fp.ring, { id: fp.id, tags: fp.tags });
  ...
  for (const batch of roadDecals) {
    ...
    const mesh = new THREE.Mesh(
      merged,
      getRoadMaterial(batch.color, batch.color <= 0x646a73)
    );
  ...
  const bridgeDecks = [];
  {
    ...
    for (const br of bridges) {
      ...
      scene.add(new THREE.Mesh(rg, railMat));
      {
        ...
        scene.add(new THREE.Mesh(gg, getSimpleStandard(0x4a5058, { roughness: 0.8 })));
      }
  ...
  // ---- STATIONS ----
  {
    ...
    for (const st of stations.slice(0, 6)) {
      ...
      sp.scale.set(worldW, worldW * (96 / c2.width), 1);
      sp.position.set(st.x, gy2 + 9.5, st.z);
      sp.renderOrder = 20;
      scene.add(sp);
    }
  }
    ...
    try {
      ...
      if (r3.ok) {
        ...
        if (geos2.length) {
          ...
          scene.add(new THREE.Mesh(merged2, oMat));
