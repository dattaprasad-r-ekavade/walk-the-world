...
export async function assembleCityFromBuild(built, ctx) {
  ...
  if (trees?.count) {
    // Two offset low-poly crowns read as foliage from more angles than the
    // former single cone while remaining one instanced draw call.
    const canopy = new THREE.IcosahedronGeometry(1.75, 1);
    canopy.scale(1.05, 1.18, 0.92);
    canopy.translate(-0.25, 4.05, 0.1);
    const canopyTop = new THREE.IcosahedronGeometry(1.3, 1);
    canopyTop.scale(0.9, 1.12, 1.05);
    canopyTop.translate(0.45, 5.15, -0.2);
    const trunk = new THREE.CylinderGeometry(0.22, 0.28, 2.6, 5).toNonIndexed();
    ...
    const cTopCol = new Float32Array(canopyTop.attributes.position.count * 3);
    for (let i = 0; i < cTopCol.length; i += 3) { cTopCol[i] = 0.20; cTopCol[i + 1] = 0.43; cTopCol[i + 2] = 0.24; }
    canopyTop.setAttribute("color", new THREE.BufferAttribute(cTopCol, 3));
    trunk.setAttribute("color", new THREE.BufferAttribute(tCol, 3));
    const treeGeo = mergeGeometries([trunk, canopy, canopyTop], false);
  ...
  const bridgeDecks = [];
  {
    ...
    for (const br of bridges) {
      ...
      bridgeDecks.push({ pts: P, halfW: halfW + 0.3, y0, y1, len, cum, cellKey });
    }
  }
  if (appendMeta) {
    engineRef.current.bridgeDecks = [
      ...(engineRef.current.bridgeDecks || []),
      ...bridgeDecks,
    ];
  } else {
  ...
  const tagCell = (arr) =>
    cellKey ? arr.map((item) => ({ ...item, cellKey })) : arr;
  if (appendMeta) {
    engineRef.current.spinners = [...(engineRef.current.spinners || []), ...spinners];
    engineRef.current.roadPaths = [
      ...(engineRef.current.roadPaths || []),
      ...tagCell(roadPaths),
    ];
    engineRef.current.propMarkers = [
      ...(engineRef.current.propMarkers || []),
      ...tagCell(props),
    ];
    engineRef.current.pois = [
      ...(engineRef.current.pois || []),
      ...tagCell(meta.pois || []),
    ];
    ...
    engineRef.current.roadPaths = tagCell(roadPaths);
    engineRef.current.propMarkers = tagCell(props);
    engineRef.current.pois = tagCell(meta.pois || []);
