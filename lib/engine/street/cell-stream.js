...
export function createCellStreamer(ctx) {
  ...
  async function loadCellBody(lat, lon, key) {
    ...
    try {
      ...
      // Clip to this cell's tile + skip OSM ids already in the scene.
      // Overlapping 650 m disks were stacking roads/buildings/benches on top
      // of each other (broken ribbons, props in the carriageway).
      let elements = clipElementsToCell(data.elements, near.lat, near.lon);
      ...
      failed.delete(key);
      evictFar(lat, lon);
      console.info(`[stream] loaded ${dataKey} (${elements.length} features)`);
    } catch (e) {
