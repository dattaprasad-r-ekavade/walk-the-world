...
export function createVehicleController({ insideBuilding, groundHeight }) {
  ...
  const exit = () => {
    ...
    const pose = {
      ...
      // walk fwd = (sin h, −cos h); car fwd = (sin h, +cos h) → mirror, not offset
      heading: Math.PI - state.heading, // convert to walk heading (−Z forward)
  ...
  const update = (dt, input, opts = {}) => {
    ...
    if (Math.abs(state.speed) > 0.35 || Math.abs(throttle) > 0.1) {
      ...
      // negated: +heading turns toward +X which is LEFT of +Z-forward, so
      // D (steer +1) must DECREASE heading to turn right
