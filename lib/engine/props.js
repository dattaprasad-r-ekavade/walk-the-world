...
// Kinds that must stay as live objects (animated parts)
const LIVE_KINDS = new Set(["turbine"]);

export function placeProps(props, { scene, groundHeight, spinners, lampGlows }) {
  // bake buckets: material -> world-space geometries
  const bake = new Map();
  const bakeMesh = (mesh) => {
    mesh.updateWorldMatrix(true, false);
    const g = mesh.geometry.clone().applyMatrix4(mesh.matrixWorld);
    if (!bake.has(mesh.material)) bake.set(mesh.material, []);
    bake.get(mesh.material).push(g);
  };
...
const sharedGlow = new THREE.MeshBasicMaterial({ color: 0x1a1c20 }); // ONE material for all bulbs
const glowMat = () => sharedGlow;
let currentKindLive = false;
const add = (mesh, x, z, y = 0) => {
  mesh.position.set(x, groundHeight(x, z) + y, z);
  if (currentKindLive) {
    scene.add(mesh); // animated props stay live
    return mesh;
  }
  // static props: bake every sub-mesh into per-material buckets
  scene.add(mesh); // temporarily attach for world matrices
  mesh.updateWorldMatrix(true, true);
  mesh.traverse((o) => { if (o.isMesh) bakeMesh(o); });
  scene.remove(mesh);
  return mesh;
};
...
for (const pr of props) {
  ...
  if (b) {
    currentKindLive = LIVE_KINDS.has(pr.kind);
    b(pr.x, pr.z, pr.tags);
    placed++;
  }
}
// flush bake buckets: hundreds of props -> ~10 draw calls
let bakedMeshes = 0;
for (const [mat, geos] of bake) {
...
if (bake.size) console.info(`[props] baked ${placed} props into ${bakedMeshes} meshes`);
}
