...
function clientOverpassQuery(lat, lon) {
  ...
  return [
    ...
    `(node(${around})[natural=tree];node(${around})[highway~"^(street_lamp|bus_stop|traffic_signals)$"];node(${around})[amenity~"^(bench|fountain|waste_basket)$"];node(${around})[shop];node(${around})[amenity];); out 500;`,
  ].join("");
}
