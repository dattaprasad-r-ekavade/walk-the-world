...
async function fromLocalStorage(cacheKey) {
  try {
    ...
    if (hit?.v === LOCAL_CACHE_SCHEMA && Date.now() - hit.t < 7 * 864e5) return hit.d;
  } catch {
    /* ignore */
  }
  return null;
...
function rememberLocal(cacheKey, data) {
  try {
    localStorage.setItem(cacheKey, JSON.stringify({ v: LOCAL_CACHE_SCHEMA, t: Date.now(), d: data }));
  } catch {
    /* quota */
  }
}
