...
/**
 * Prefer a nearby already-warm R2/localStorage cell, but ONLY if it is close
 * to the requested point. Reusing a warm cell ~1 km away was the walk bug:
 * streamer clipped to that far center, got 0 features at the player, and
 * permanently marked the slot loaded — so roads/buildings never appeared.
 *
...
export async function fetchCityDataNear(lat, lon) {
  ...
  // ~450 m — inside one Overpass disk; beyond this we must fetch the real cell.
