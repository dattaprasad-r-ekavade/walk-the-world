...
export function createPopulation({ scene, groundHeight, roadPaths, pois = [] }) {
  // ---- street graph ----
  ...
  if (walkable.length) {
    ...
    for (let i = 0; i < PED_MAX; i++) {
      ...
      peds.setColorAt(i, new THREE.Color(PED_COLORS[i % PED_COLORS.length]));
    }
  ...
  if (drivable.length) {
    ...
    for (let i = 0; i < CAR_MAX; i++) {
      ...
      cars.setColorAt(i, new THREE.Color(CAR_COLORS[i % CAR_COLORS.length]));
    }
  ...
  const birdGeo = new THREE.ConeGeometry(0.18, 0.7, 4);
  birdGeo.rotateX(Math.PI / 2);
  const birdMat = new THREE.MeshBasicMaterial({ color: 0x2c2f36 });
    ...
    if (peds) {
      for (let i = 0; i < PED_MAX; i++) {
        ...
        if (i >= pedActive) {
          ...
          if (p.d > p.w.L || p.d < 0) {
            // reached the end: turn around or hop to another street
            if (Math.random() < 0.4) {
          ...
          dummy.position.set(px, groundHeight(px, pz) + Math.abs(Math.sin(p.phase)) * 0.05, pz);
    ...
    if (cars) {
      for (let i = 0; i < CAR_MAX; i++) {
        ...
        if (i >= carActive) {
          ...
          if (ccar.d > ccar.w.L) ccar.d = ccar.dir > 0 && ccar.w.oneway ? 0 : (ccar.dir *= -1, ccar.w.L);
          if (ccar.d < 0) { ccar.dir *= -1; ccar.d = 0; }
          const s = sampleAt(ccar.w.pts, ccar.w.cum, ccar.d);
          // drive on the left-ish lane offset relative to travel direction
          const lane = (ccar.w.width / 4) * ccar.dir;
          const px = s.x + -s.dz * lane, pz = s.z + s.dx * lane;
          dummy.position.set(px, groundHeight(px, pz) + 0.05, pz);
          dummy.rotation.set(0, Math.atan2(s.dx * ccar.dir, s.dz * ccar.dir), 0);
          dummy.scale.setScalar(1);
        }
        dummy.updateMatrix();
