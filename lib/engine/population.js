...
export function createPopulation({ scene, groundHeight, roadPaths, pois = [], models = {} }) {
  ...
  if (walkable.length) {
    ...
    const mat = models.ped?.material || new THREE.MeshLambertMaterial();
  ...
  if (drivable.length) {
    ...
    for (let i = 0; i < CAR_MAX; i++) {
      ...
      carList.push({
        ...
        h: null, // smoothed heading
        y: null, // smoothed height
      });
      ...
      for (let i = 0; i < CAR_MAX; i++) {
        ...
        if (i >= carActive) {
          ...
          if (ccar.d > ccar.w.L || ccar.d < 0) {
            // end of the road: move to a fresh street (cars never reverse —
            // ping-ponging back and forth reads as broken AI)
            if (drivable.length) {
          ...
          // smooth heading across polyline corners and height over terrain
          // seams — raw values pop every segment and read as stutter
