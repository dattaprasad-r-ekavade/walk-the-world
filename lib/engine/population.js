...
function makeNameplateSprite(label, groundHeight, x, z) {
  ...
  // sit just above storefront height — less likely to embed in tall walls
  sp.position.set(x, groundHeight(x, z) + 3.6, z);
  sp.renderOrder = 2;
  sp.userData.poiX = x;
  sp.userData.poiZ = z;
  sp.userData.dispose = () => {
...
export function createPopulation({ scene, groundHeight, roadPaths, pois = [], models = {}, signals = [] }) {
  ...
  // ---- POI sign sprites (named shops/amenities; visibility gated by distance) ----
