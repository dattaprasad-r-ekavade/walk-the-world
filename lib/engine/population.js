...
export function placeOutsideBuilding(x, z, insideBuilding, opts = {}) {
  ...
  if (alreadyOut && preferW) {
    ...
    if (!insideBuilding(nx, nz)) return { x: nx, z: nz, moved: true };
    return { x, z, moved: false };
  }
  if (alreadyOut) return { x, z, moved: false };
  ...
  if (preferW) {
    for (let r = 1.5; r <= maxR; r += 1.1) {
      const hit = tryPoint(x + preferX * r, z + preferZ * r);
      if (hit) return { ...hit, moved: true };
      // fan ±60° around preferred direction
      for (const ang of [-0.6, 0.6, -1.1, 1.1]) {
        const c = Math.cos(ang);
        const s = Math.sin(ang);
        const dx = preferX * c - preferZ * s;
        const dz = preferX * s + preferZ * c;
        const h2 = tryPoint(x + dx * r, z + dz * r);
        if (h2) return { ...h2, moved: true };
      }
    }
  }
  ...
  for (let r = 1.5; r <= maxR; r += 1.25) {
    ...
    for (let i = 0; i < steps; i++) {
      ...
      const hit = tryPoint(x + Math.cos(a) * r, z + Math.sin(a) * r);
      if (!hit) continue;
      const d = Math.hypot(hit.x - x, hit.z - z);
      // slight preference for road-facing side
    ...
    if (best) return { ...best, moved: true };
  }
  return { x, z, moved: false };
}
...
function makeNameplateSprite(label, groundHeight, x, z) {
  ...
  // depthTest on so plates hide behind buildings (not always-on-top)
  ...
  // storefront height, outside the wall
  sp.position.set(x, groundHeight(x, z) + 3.2, z);
  sp.renderOrder = 2;
  sp.userData.poiX = x;
  sp.userData.poiZ = z;
  sp.userData.dispose = () => {
...
export function createPopulation({
  ...
  for (const p of named.slice(0, SIGN_MAX)) {
    ...
    const sp = makeNameplateSprite(p.name, groundHeight, placed.x, placed.z);
    sp.visible = false;
  ...
  const releaseCar = (index, pose) => {
    ...
    if (pose) {
      c.px = pose.x;
      c.pz = pose.z;
      // pose.heading is walk/drive (−Z); store mesh-facing (+Z) for AI resume
  ...
  const setDrivenPose = (index, pose) => {
    ...
    c.px = pose.x;
    c.pz = pose.z;
    // Walk/drive heading 0 = −Z; mesh +Z forward → add π so the body turns with the car
    c.h = pose.heading + Math.PI;
    c.y = pose.y;
