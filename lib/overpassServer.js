...
export async function fetchOverpassCell(lat, lon) {
  ...
  // Partial results beat no results on serverless: if one of the two
  // queries times out, render what we have and let a later visit fill it.
  const [bRes, iRes] = await Promise.allSettled([
    runQuery(buildingsQuery(around)),
    runQuery(infraQuery(around)),
  ]);
  if (bRes.status === "rejected" && iRes.status === "rejected") throw bRes.reason;
  const buildings = bRes.status === "fulfilled" ? bRes.value : { elements: [] };
  const infra = iRes.status === "fulfilled" ? iRes.value : { elements: [] };
